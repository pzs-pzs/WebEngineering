...
public class JobDaoImpl implements DataDao{
	...
	public List<EndData> getData(int start, int offset) {
		...
		String sql = "Select * from tb_job  order by time desc limit "+start+","+offset;
	...
	public EndData getData(String id) {
			...
			String sql = "Select * from tb_job where id="+id;
			ResultSet rs = conn.executeQuery(sql);
			DateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			EndData temp = new EndData();
			try {
	...
	public boolean storeDate(EndData data) {
		String sql = "insert into tb_job (title,username,time,text) value "
